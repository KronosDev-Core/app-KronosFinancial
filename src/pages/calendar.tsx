...
import { roundUp, sub } from '@Local/utils/Math';
...
const Calendar: FC = (): JSX.Element => {
  ...
  return (
    <div className="w-full h-full grid grid-cols-6 grid-rows-6 bg-slate-700 gap-4 p-2 lining-nums tabular-nums text-slate-200">
      <div className="row-span-4 col-span-2 bg-slate-800" />
      <div className="bg-slate-800 rounded-lg grid grid-rows-6 grid-cols-1 p-4 row-span-6 col-span-4">
        ...
        <div
          className={`row-span-5 col-span-1 grid grid-flow-row auto-rows-fr grid-cols-7 gap-2`}
        >
          {Array.from({ length: totalDays }, (_, i) => {
            ...
            return (
              <CalendarItem
                key={i}
                date={firstDay.add(i, 'days') as DayJsType}
              />
            );
          })}
        </div>
